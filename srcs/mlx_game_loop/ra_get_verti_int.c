...
static myfloat	get_first_intersection_x(t_cub *cub)
{
	float	inter_x;
...
static myfloat	get_first_intersection_y(t_cub *cub, float first_itner_x)
{
	float	hor_dist;
...
static myfloat	inside_adjust_x(t_cub *cub)
{
	if (look_right(cub->ray->angle))
		return (1);
	else
