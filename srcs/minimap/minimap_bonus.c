...
void	init_minimap_bonus(t_cub *cub) {
  ...
  cub->mini.img_ptr =
      mlx_new_image(cub->mlx.mlx, cub->mini.width, cub->mini.height);
  cub->mini.img_data = (int *)mlx_get_data_addr(
      cub->mini.img_ptr, &cub->mini.bpp, &cub->mini.size_l, &cub->mini.endian);
}
